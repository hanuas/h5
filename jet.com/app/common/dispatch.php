...
function __autoload($class_name) {
    ...
    Dispatch::load( $class_name ); 
    
	if(Func::endWith($class_name, 'Controller')){
		Dispatch::loadController( Dispatch::curModule() ,substr($class_name,0,strlen($class_name)-10));  
	}
...
function include_third($ref_path, $once = true, $require = false){
	Dispatch::include_third( $ref_path, $once , $require);
}
...
class Dispatch{
	...
	{
		return Dispatch::loadClassCommon(
				$className	,
	...
	public static function getFilePathByclassName($className , $relativePathOfApp ){
		if(empty($relativePathOfApp)){
			...
			if(self::str_end_with($className,"Model")){
				...
				$file = _SERVICE_DIR_.str_replace("_","/", substr($className , 0 , strlen($className)-7) ).".service.php";
			}else{
...
class DRouter {
	...
	*	Static 类型，默认
	*/
	//若要支持带冒号前缀的变量方式（RESTFUL 的接口要用到），可以用rewrite方式实现
	function dealStatic(){
