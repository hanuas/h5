<?php
//
class UserReadMsgService{

    //
    public function logReadMsgLastTime($user_id,$game_id,$msg_type){
        $model = new UserReadMsgModel();
        $time = date("Y-m-d H:i:s");
        //检查是否有记录
        $where = "user_id = {$user_id} and game_id={$game_id} and msg_type = '{$msg_type}'";
        $info = $model->readInfo('id',$where);
        if($info){  //有记录，修改
            $res = $model->updateById(array('last_read_time'=>$time),$info['id']);
        }else{ //没记录，添加
            $addData = array(
                'game_id'=>$game_id,
                'user_id'=>$user_id,
    ...
    public function getUnReadNormalGiftCount($user_id,$game_id){
        ...
        $giftModel = new GiftModel();
        $where = "game_id = {$game_id} and get_type in ('normal','qq_group_num','union_code') and gift_status = 1 and start_time>'{$lastReadTime}'";
        return $giftModel->getGiftCount($where);
    }
    ...
    //检查是否有新积分礼包
    public function checkHasNewPointGift($game_id){
        $giftModel = new GiftModel();
        $time = date("Y-m-d H:i:s");
        $new_time_range = date('Y-m-d H:i:s',time()-60*60*24*2); //2天之内上架的算新礼包
        $where = "game_id = {$game_id} and get_type = 'point'  and start_time <'{$time}' and start_time >'{$new_time_range}' and end_time > '{$time}' and gift_status = 1";
        $count = $giftModel->getGiftCount($where);
        return $count>0;
    }
