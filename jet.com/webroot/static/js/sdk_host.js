...
(function() {
    ...
    sdk.wxappId = "wxfb7a886d5e56d43e";
    sdk.wxopenId = "wx22f69b39568e9cb3";
    sdk.qqappId = "1106426949";
    sdk.gameId = 0;
	...
	sdk.apiHost = "https://gh5api.ktsdk.com";
	sdk.userHost = "https://gh5api.ktsdk.com";
	sdk.loginHost =  "https://gh5api.ktsdk.com";
	sdk.webHost =   "https://gh5api.ktsdk.com";
	sdk.cdnHost =   "https://gh5api.ktsdk.com";
	sdk.onlineHost =   "https://gh5api.ktsdk.com";
	sdk.playHost =   "https://gh5api.ktsdk.com";
	sdk.gameHost =   "https://gh5api.ktsdk.com";
    ...
    sdk.getCommonArgs = function () {
    	...
    	var osVersion = "1.0";
    	var devId = sdk.getURLVar("device")?sdk.getURLVar("device"):"";
    	var adv_channel = sdk.getURLVar("chid")?sdk.getURLVar("chid"):"official";
        var subchid = sdk.getURLVar("subchid")?sdk.getURLVar("subchid"):"";
    	var app_version = "1.0";
        var game_id = sdk.gameId;
        if(!game_id){
            game_id=sdk.getURLVar("gameid")?sdk.getURLVar("gameid"):0;
        }
    ...
    sdk.initHost = function (callback) {
        ...
        function __checkTokenKey() {
            var url = sdk.apiHost + "/api/login/getTokenByTokenKey?tokenkey=" + sdk.getURLVar("tokenkey");
    ...
    sdk.createCode = function (token, callback) {
        var url = sdk.apiHost + "/api/login/getCodeByToken?token=" + token + "&" + Date.now();
        url = sdk.setURLArgs(url,sdk.getCommonArgs());
        $.get(url, function (data) {
            callback(data);
        }, "json");
    ...
    sdk.checkCode = function (code, callback) {
        var url = sdk.apiHost + "/api/login/checkCode?code=" + code + "&" + Date.now();
        url = sdk.setURLArgs(url,sdk.getCommonArgs());
        $.get(url, function (data) {
            callback(data);
        }, "json");
    ...
    sdk.getUserToken = function (token, callback) {
        var url = sdk.apiHost + "/api/login/getUserToken?token=" + token + "&" + Date.now();
        url = sdk.setURLArgs(url,sdk.getCommonArgs());
        $.get(url, function (data) {
            callback(data);
        }, "json");
    ...
    sdk.checkToken = function (token, callback) {
        var url = sdk.apiHost + "/api/login/checkToken?token=" + token + "&" + Date.now();
        url = sdk.setURLArgs(url,sdk.getCommonArgs());
        $.get(url, function (data) {
            callback(data);
        }, "json");
        ...
        $.get(sdk.webHost + "/api/share/getShareDomain?gameid=" + sdk.gameId + "&token=" + sdk.token + "&" + Date.now(), function (data) {
            ...
            if (sdk.isWeixin()) {
                ...
                */
                var data = {
                    title:          sdk.getShareDomain.text,
                    desc:        sdk.getShareDomain.text,
                    image_url:            sdk.getShareDomain.image,
                    share_url:            shareURL
                };
                //window.mqq.invoke("data","setShareInfo",share_data);
    ...
    sdk.appShare = function(args){
		...
		window.__shareCallback = function(){
			alert("分享成功");
		}

        if(sdk.isAndroid()) {
			window.android.showAppShare(JSON.stringify(shareData), "__shareCallback");
        }else if(sdk.isiOS()){
            sdk.initWebViewJavascriptBridge(function () {
                if (window.WebViewJavascriptBridge) {
                    WebViewJavascriptBridge.callHandler("showAppShare",shareData, function () {
						__shareCallback();
                    });
                }
            });
    ...
    sdk.loadPayBox = function (args, wxpayCallback, alipayCallback, qqpayCallback) {
        ...
        $.get(sdk.apiHost + "/api/conf/getProductInfo?pid=" + args["pid"] + "&game_id="+ sdk.gameId +"&" + Date.now(), function (data) {
    ...
    sdk.wxPay = function (args, callback) {
        if (args) {
            var url = sdk.buildURL(sdk.loginHost + "/pay/wxpay/js_api_call", {
                gameid: sdk.gameId,
                uid: sdk.uid,
                token:sdk.token,
                product_id: args["pid"],
                ...
                txid: args["txid"],
                areaID:args['areaID'],//大区id
                serverID:args['serverID'],//服务器id
                serverName:args['serverName'],//服务器名
                roleID:args['roleID'],//角色id
                roleName:args['roleName'],//角色名
                roleLevel:args['roleLevel'],//角色等级
                accountID:args['accountID'],//游戏账号id
                extendbox:args['extendbox'],//扩展参数，用于支付通知发货
                payext:args['payext'],//扩展参数，用于支付通知发货
                _v: Date.now()
            });
            url = sdk.setURLArgs(url,sdk.getCommonArgs());
    ...
    sdk.wxAppPay = function (args, callback) {
        ...
        */
        ...
        $.get(url, function (data) {
            var payData = {
                partnerId: sdk.isJetboxAPP() ? "1490616912" : "1490616912",
    ...
    sdk.qqPay = function (args, callback) {
        function __qqPay() {
            ...
            $.get(url, function (data) {
                sdk.hideLoading();
                if (data.error) {
                    return alert('建立订单失败：' + data.error);
                }
                mqq.tenpay.pay({
    ...
    sdk.wxh5Pay = function (args, callback, transCallback) {
        ...
        var url = sdk.buildURL(sdk.loginHost + "/pay/weixin/h5", {
            ...
            extendbox:args['extendbox'],//扩展参数，用于支付通知发货
            payext:args['payext'],//扩展参数，用于支付通知发货
            //backurl: encodeURI(location.href),
            _v: Date.now()
        });
        url = sdk.setURLArgs(url,sdk.getCommonArgs());
        var number = args["product_count"];
    ...
    sdk.alih5Pay = function (args, callback) {
        sdk.aliPay(args, callback, function (url) {
            var url = sdk.buildURL(sdk.loginHost + "/pay/alipay", {
                gameid: sdk.gameId,
                uid: sdk.uid,
                token:sdk.token,
                product_id: args["pid"],
    ...
    sdk.aliPay = function (args, callback, transCallback) {
        ...
        url = sdk.setURLArgs(url,sdk.getCommonArgs());
        url = sdk.setURLArgs(url,{"backurl":location.href});
        var number = args["product_count"];
        if (number) {
            url += ("&product_count=" + number);
        }
    ...
    sdk.downJetApp = function () {
        if (sdk.isTrial) {
            ...
            if (sdk.isAndroid()) {
                location.href = 'https://a.app.qq.com/o/simple.jsp?pkgname=com.jetapp.app';
            } else if (sdk.isiOS()) {
    ...
    sdk.saveGameDesktop = function () {
        if (sdk.isiOS() && sdk.isMobile()) {
            var url = sdk.apiHost + '/api/login/getTokenKey?token=' + sdk.token;
            url = sdk.setURLArgs(url,sdk.getCommonArgs());
            $.get(url, function (data) {
                if (data.tokenkey) {
    ...
    sdk.loadTrialInfo = function () {
        ...
        t += '</div>';
        $("body").append(t);
        var url = sdk.apiHost + '/api/login/getUserByToken?token=' + sdk.token;
        url = sdk.setURLArgs(url,sdk.getCommonArgs());
        $.get(url, function (data) {
            if (data.nickname) {
                try {
    ...
    sdk.enableExitBanner = function () {
        var $history = $('<script />', {
            src: sdk.cdnHost + '/static/js/sdk_history.js?v=' + Date.now()
        });
    ...
    sdk.enableFloatBox = function () {
        var $float = $('<script />', {
            src: sdk.cdnHost + '/static/js/sdk_float.js?v=' + Date.now(),
            id: 'ftBoxJs'
        });
        if (!$("#ftBoxJs").length) {
            $("body").append($float);
    ...
    sdk.mobileParamsCheck = function (args, callback) {
        if (!args.jetUser && args.jetUser !== undefined) {
            ...
            if (!(/^1[3|4|5|7|8|9]\d{9}$/.test(args.phone)) && args.phone !== undefined) {
